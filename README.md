...
- 获取锁
  - 互斥：确保只能有一个线程获取锁
  - 非阻塞：尝试一次，成功返回 true ,失败就返回 False 
- 释放锁
  - 手动释放
  - 超时释放：获取锁时添加一个超时时间
...
问题1 ： 在释放锁的时候，会出现线程1阻塞之后，线程2获取了锁，他把线程1的锁给释放了

因此要在释放所之前，加一个判断就是，是否是当前线程的锁，如果是就del释放，否则就不能释放。
...
问题2 ：锁释放和判断是否是当前线程的锁，这两个动作之间，有可能因为JVM在执行GC任务时将线程阻塞，导致释放锁推迟，

而另一个线程2获取到锁，执行了他的任务。 要保证原子性。

![image-20240207103131952](./README_images/image-20240207103131952.png)
...
需求 ： 基于 Lua 脚步实现分布式锁的释放锁的逻辑

提示 ： RedisTemplate 调用

 

- 利用 set nx ex 获取锁，并设置过期时间，保存线程表示
- 释放锁时线判断线程标识是否与自己一致，一致则删除 
